...
export function BudgetsPage() {
  ...
  const byCat = monthTxns.reduce((acc:any,t)=>{ acc[t.category]=(acc[t.category]||0)+Math.abs(t.amount); return acc;},{});

  function add() {
    addBudget({ month, category, limit: Number(limit||0) });
    setCategory("General"); setLimit("500");
  }
            ...
            <div key={b.id} style={{margin:'8px 0'}}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 80px', gap:10, alignItems:'center'}}>
                <div style={{background:'#0b1324', border:'1px solid #26334a', borderRadius:12, overflow:'hidden'}}>
                  <div style={{height:12, width:`${pct}%`, background: pct>90 ? '#ef4444' : '#22c55e' }} />
                </div>
                <div style={{textAlign:'right'}}>${actual.toFixed(0)} / ${b.limit.toFixed(0)}</div>
              </div>
            </div>
          );
        })}

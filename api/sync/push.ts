...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { userId, txns = [], budgets: bgs = [], goals: gls = [], profile, aiLogs: aiLogPayload = [] } = req.body || {};
  ...
  try {
    ...
    if (aiLogPayload.length) await db.insert(aiLogs).values(aiLogPayload.map((log:any)=>({ ...log, userId })));

    if (profile) {
      await db
        .insert(profiles)
        .values({ ...profile, userId })
        .onConflictDoUpdate({ target: profiles.userId, set: { ...profile, updatedAt: new Date() } });
    }

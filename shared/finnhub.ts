...
const FALLBACK_FINNHUB_API_KEY = "d4dkf41r01qmhtc4rqd0d4dkf41r01qmhtc4rqdg";
...
const resolveApiKey = (apiKey?: string | null) => {
  const key = apiKey ?? process.env.FINNHUB_API_KEY ?? FALLBACK_FINNHUB_API_KEY;
  if (!key) {
    throw new FinnhubError("Market data API key not configured", 500);
  }
  if (!process.env.FINNHUB_API_KEY && key === FALLBACK_FINNHUB_API_KEY) {
    console.warn("FINNHUB_API_KEY missing, using fallback key from repo.");
  }
  return key;
};
